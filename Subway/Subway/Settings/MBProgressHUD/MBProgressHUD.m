...
#define HUD_PADDING 0.0f
// see Moodjo-Prefix.pch for other constants
...
- (id)initWithFrame:(CGRect)frame {
	...
	if (self) {
        ...
        self.labelFont = [UIFont fontWithName:FONT_HUD_BIG_LABEL size: FONT_SIZE_HUD_BIG_LABEL];
        self.detailsLabelFont = [UIFont fontWithName:FONT_HUD_DETAILS_LABEL size: FONT_SIZE_HUD_DETAILS_LABEL];
        self.xOffset = 0.0f;
        self.yOffset = 0.0f;
		self.dimBackground = NO;
		self.margin = HUD_MARGIN;
		self.graceTime = 0.0f;
...
- (void)layoutSubviews {
    ...
    // Position the indicator at the center
    ...
    indicator.backgroundColor = [UIColor clearColor];  // addition Seb
	
    // Add label if label text was set
    if (nil != self.labelText) {
        
        
        // ========== HERE SEB HAS MADE SOME MODIFICATIONS TO GET BETTER LAYOUT ==========
        // (nothing has been erased, only put into comment)
        ...
        //            lHeight = lHeight + dims.height;
        //        }
        //        
        //        if (lWidth > 150) {
        //            lHeight = lHeight + dims.height + lHeight;
        //        }
		
        
        // ---------------- Seb : New way of calculating size of the label
        
        UIFont *fontSD = [UIFont fontWithName:FONT_HUD_BIG_LABEL size:FONT_SIZE_HUD_BIG_LABEL];
        CGSize sizeMax = {250.0f, 500.0f}; // contraintes max de la taille du label
        CGSize dims = [self.labelText sizeWithFont:fontSD constrainedToSize:sizeMax lineBreakMode:UILineBreakModeWordWrap];
        ...
        label.opaque = NO;
        label.numberOfLines = 0;                         // addition Seb
        label.lineBreakMode = UILineBreakModeWordWrap;   // addition Seb
        label.backgroundColor = [UIColor clearColor];    // addition Seb
        ...
        //        }
        //        self.height = self.height + lHeight + HUD_PADDING;
		
        // Move indicator to make room for the label
        //        indFrame.origin.y -= (floorf(lHeight / 2 + HUD_PADDING / 2));
        //        indicator.frame = indFrame;
		
        // Set the label position and dimensions
        //        CGRect lFrame = CGRectMake(floorf((frame.size.width - lWidth) / 2) + xOffset,
        //                                   floorf(indFrame.origin.y + indFrame.size.height + HUD_PADDING),
        //                                   lWidth, lHeight);
        //        label.frame = lFrame;
        ...
        // ---------------- Seb : New way of calculating the update of HUD size
        ...
        int space;
        if ( indFrame.size.width == 0 || indFrame.size.height == 0 )  { space = 0; } else { space = HUD_INTERSPACE; }
        
        
        self.height = self.height + lHeight + space; // add label height + space between label and indicator
		
        // Move indicator to make room for the label
        indFrame.origin.y -= ( floorf( lHeight / 2 + space / 2) );
        indicator.frame = indFrame;
		
        
        CGRect lFrame = CGRectMake( floorf( (frame.size.width - lWidth) / 2 ) + xOffset,
                                   floorf( indFrame.origin.y + indFrame.size.height + space ), // space between indic & lbl
                                   lWidth, lHeight );
        label.frame = lFrame;
        ...
        if (nil != self.detailsLabelText) {
            ...
            self.height = self.height + lHeight + HUD_INTERSPACE;
			
            // Move indicator to make room for the new label
            indFrame.origin.y -= (floorf(lHeight / 2 + HUD_INTERSPACE / 2));
            indicator.frame = indFrame;
			
            // Move first label to make room for the new label
            lFrame.origin.y -= (floorf(lHeight / 2 + HUD_INTERSPACE / 2));
            label.frame = lFrame;
			
            // Set label position and dimensions
            CGRect lFrameD = CGRectMake(floorf((frame.size.width - lWidth) / 2) + xOffset,
                                        lFrame.origin.y + lFrame.size.height + HUD_INTERSPACE, lWidth, lHeight);
